...

{
  ...
  mkShellHook = { models, cacheDir ? null }:
    ...
    in ''
      ...
      ${lib.concatMapStrings ({ org, model, ... }: ''
        echo "  - ${org}/${model}"
      '') models}
    '';
  ...
  mkHfSymlinks = {
    ...
    in pkgs.writeShellScript "setup-hf-symlinks-${org}-${model}" ''
      set -euo pipefail
  ...
  mkModelWrapper = {
    ...
    in pkgs.writeShellScriptBin wrapperName ''
      ${lib.concatMapStrings ({ path, envVar, ... }: ''
        export ${envVar}="${path}"
      '') models}
  ...
  mkHfCachePath = { org, model, cacheDir ? "$HOME/.cache/huggingface/hub" }:
